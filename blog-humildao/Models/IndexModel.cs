    ...
    public static class IndexModel{
        public static IList<Post> getAllPosts(){
            ...
            for (int x = 0; x < query.Count; x++){
                ...
                string selectCategorias = "select categorias.nome from posts_categorias left join categorias on posts_categorias.id_categoria = categorias.id where posts_categorias.id_post =" + query[x]["id"];
